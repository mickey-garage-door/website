    ...
    <>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-2.5 text-sm text-gray-500">
        <div className="max-w-7xl mx-auto flex items-center gap-2">
          ...
          <span>/</span>
          <Link href="/locations" className="hover:text-brand-dark">Locations</Link>
          <span>/</span>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-10 items-start">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-brand-cyan font-semibold text-sm">{location.city}, {location.state}</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-black mb-4 leading-tight">{location.headline}</h1>
            <p className="text-blue-100 text-lg mb-6 leading-relaxed">{location.subheadline}</p>
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <a
                href={`tel:${company.phonePlain}`}
                className="inline-flex items-center justify-center gap-2 bg-white text-brand-dark font-black px-6 py-4 rounded-xl hover:bg-blue-50 transition-colors"
              >
              ...
              <Link href="/contact" className="inline-flex items-center justify-center bg-orange-500 text-white font-black px-6 py-4 rounded-xl hover:bg-orange-600 transition-colors">
                Free Quote
              </Link>
            </div>
            <div className="flex flex-wrap gap-2">
              {location.neighborhoods.map((n) => (
                <span key={n} className="text-xs bg-white/10 text-blue-100 rounded-full px-3 py-1 border border-white/20">{n}</span>
              ))}
            </div>
          </div>
      ...
      <section className="py-12 px-4 bg-white">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-8">
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-4">Garage Door Services in {location.city}, CA</h2>
            ...
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-5">Services Available in {location.city}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {services.map((service) => (
                  <Link
                    key={service.slug}
                    href={`/services/${service.slug}`}
                    className="group flex items-center gap-3 border border-gray-200 rounded-lg p-4 hover:border-brand-dark hover:bg-blue-50 transition-all"
                  >
                    <span className="text-xl shrink-0">{service.icon}</span>
                    <div>
                      <p className="font-semibold text-gray-900 text-sm group-hover:text-brand-dark transition-colors">{service.shortTitle}</p>
            ...
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-5">{location.city} Garage Door FAQs</h2>
          ...
          <div className="space-y-6">
            <div className="bg-brand-gradient text-white rounded-xl p-6">
              ...
              <p className="text-blue-100 text-sm mb-4">{company.emergency}</p>
              <a
                href={`tel:${company.phonePlain}`}
                className="w-full flex items-center justify-center gap-2 bg-white text-brand-dark font-black py-3 rounded-lg hover:bg-blue-50 transition-colors"
              >
              ...
              <p className="text-blue-200 text-xs text-center mt-3">{company.hours}</p>
            </div>
      ...
      <section className="py-12 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-black text-gray-900 mb-6">What {location.city} Customers Say</h2>
