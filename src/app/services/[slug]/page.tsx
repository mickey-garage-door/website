...
export default async function ServicePage({ params }: Props) {
  ...
  const serviceSchema = {
    ...
    "@type": "Service",
    name: service.title,
    provider: { "@id": "https://mickeysgaragedoor.com/#business" },
    ...
    <>
      ...
      <nav className="bg-gray-50 border-b border-gray-200 px-4 py-2.5 text-sm text-gray-500">
        <div className="max-w-7xl mx-auto flex items-center gap-2">
          ...
          <span>/</span>
          <Link href="/services" className="hover:text-brand-dark">Services</Link>
          <span>/</span>
      ...
      <section className="bg-brand-gradient-hero text-white py-14 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-black mb-4 leading-tight">{service.headline}</h1>
          ...
          <div className="bg-white dark:bg-surface text-gray-900 dark:text-foreground rounded-2xl p-6 shadow-2xl border border-gray-100 dark:border-gray-700 transition-colors duration-300">
            <h2 className="text-xl font-bold text-brand-dark dark:text-brand-cyan mb-1">Request a Free {service.shortTitle} Estimate</h2>
      ...
      <section className="py-14 px-4 bg-white">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-4">About {service.shortTitle} Service in San Diego</h2>
              <p className="text-gray-600 leading-relaxed text-base">{service.description}</p>
            </div>

            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-4">Signs You Need {service.shortTitle}</h2>
              ...
              <div className="space-y-4">
                {service.process.map((step, idx) => (
                  <div key={step.step} className="flex gap-4 items-start">
                    <div className="w-9 h-9 rounded-full bg-brand-gradient text-white font-black text-sm flex items-center justify-center shrink-0">
                      {idx + 1}
                    </div>
                    <div>
          ...
          <div className="space-y-6">
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-6">
              <h3 className="font-black text-brand-dark text-lg mb-2">Pricing</h3>
              <p className="text-gray-700 text-sm mb-4">{service.priceRange}</p>
              <a
                href={`tel:${company.phonePlain}`}
                className="w-full flex items-center justify-center gap-2 bg-brand-dark text-white font-bold py-3 rounded-lg hover:bg-brand-mid transition-colors text-sm"
              >
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {service.brands.map((brand) => (
                  <span key={brand} className="text-xs bg-white border border-gray-200 rounded-full px-3 py-1 text-gray-600">{brand}</span>
                ))}
              </div>
            </div>
        ...
        <section className="py-12 px-4 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {related.map((rel) => (
                <Link key={rel.slug} href={`/services/${rel.slug}`} className="group bg-white border border-gray-200 rounded-xl p-5 hover:border-brand-dark hover:shadow-md transition-all">
                  <div className="text-2xl mb-2">{rel.icon}</div>
                  <h3 className="font-bold text-gray-900 group-hover:text-brand-dark transition-colors">{rel.shortTitle}</h3>
